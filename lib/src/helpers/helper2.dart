...
import 'package:ant_media_flutter/src/utils/websocket.dart';
import 'package:flutter_webrtc/flutter_webrtc.dart';

//if (dart.library.js) '../../src/utils/websocket_web.dart';
...
typedef void PeerHelperStateCallback(Helper2State state);
...
class AntHelper2 extends Object {
  MediaStream? _localStream;
  List<MediaStream> _remoteStreams = [];
  PeerHelperStateCallback onStateChange;
  ...
  void invite(String peerId, String media, useScreen) {
    this.onStateChange(Helper2State.CallStateNew);
  ...
  connect() async {
    ...
    _socket?.onOpen = () {
      print('onOpen');
      this.onStateChange(Helper2State.ConnectionOpen);
      if (_type == "peer") {
        join(_streamId);
      } else {
        joinroom(_streamId);
    ...
    _socket?.onClose = (int code, String reason) {
      ...
      this.onStateChange(Helper2State.ConnectionClosed);
    };
  ...
  _closePeerConnection(streamId) {
    ...
    this.onStateChange(Helper2State.CallStateBye);
  }
